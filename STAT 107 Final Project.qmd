...
```{r}
Data <- read.csv("data/career.csv")
head(Data[,c("Career.Change.Interest","Gender", "Family.Influence","Current.Occupation","Job.Security")])
```

```{r}
Data$Family.Influence <- factor(Data$Family.Influence,
                              c("None","Low","Medium","High"))

Data$Current.Occupation <- factor(Data$Current.Occupation,
...
```{r}
table1(Data,
       Gender,Family.Influence,Current.Occupation,Job.Security,
       splitby = ~Career.Change.Interest,
       row_wise = TRUE,
       test = TRUE)
```
